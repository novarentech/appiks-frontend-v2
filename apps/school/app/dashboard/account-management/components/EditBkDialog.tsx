...
type BkFormValues = z.infer<typeof bkSchema>;

interface EditBkDialogProps {
  open: boolean;
...
export function EditBkDialog({
  ...
  // Pre-fill form when bk changes
  React.useEffect(() => {
    if (open && bk) {
      form.reset({
        name: bk.name || "",
        username: bk.username || bk.name?.toLowerCase().replace(/\s+/g, "_") || "",
        role: bk.role || "Guru BK",
        phone: stripCountryCode(bk.phone),
        nip: bk.nip || "",
        ...
        <DialogHeader>
          <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4 text-center sm:text-left pb-2">
            ...
            <div className="space-y-1">
              <DialogTitle>Edit Akun Guru BK</DialogTitle>
              <DialogDescription>
